...
class ProductController {
    ...
    // ADD PRODUCTS METHOD controller 
    ...
    static deleteAllProducts = async (req, res) => {
        try {
            ...
            console.log(data)
            if (data.length === 0) {
                return res.json({ status: API_STATUS_CODES.ERROR_CODE, message: RESPONSE_MESSAGES.AUTHORIZATION_FAILED, message: "No products exists in database against this seller.", data: data });
            }
            // DELETE ALL PRODUCTS OF SPEIFIC SELLER
            else {
                const data2 = await ProductService.deleteAllProducts(seller_id);
                return res.json({ status: API_STATUS_CODES.SUCCESS, message: RESPONSE_MESSAGES.SUCCESS, message: "product deleted succesfully against seller", dataa2: data2 });
            }

        } catch (err) {
            console.log(err)
            return res.json({ status: API_STATUS_CODES.INTERNAL_SERVER_ERROR, message: "An error occured in delteAllproducts API." })
        }
    ...
    static deleteProductById = async (req, res) => {
        try {
            ...
            console.log(err);
            return res.json({ status: API_STATUS_CODES.INTERNAL_SERVER_ERROR, message: "An error occured in deleteProductById." })
        }
    ...
    static editProduct = async (req, res) => {
        try {
            ...
            console.log(err);
            return res.json({ status: API_STATUS_CODES.INTERNAL_SERVER_ERROR, message: "An error occured in editProduct API." })
        }
